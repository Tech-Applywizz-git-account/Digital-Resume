...
serve(async (req) => {
    ...
    try {
        ...
        const { resumeText, messages, question } = body;

        if (!resumeText || !question) {
            throw new Error("Missing resumeText or question in request body");
        }

        console.log("Processing question with history length:", messages?.length || 0);

        // Construct the conversation history
        const conversationMessages = [
            {
                role: "system",
                content: `You are a helpful AI assistant analyzing a resume.
