    ...
    for (const crmUser of (crmUsers as CRMUser[]) || []) {
      try {
        ...
        if (existingCRMRecord) {
          console.log(`User ${crmUser.email} already exists`);
          results.push({
            email: crmUser.email,
        ...
        if (authError) {
          ...
          if (authError.message.includes("already been registered")) {
            console.log(`User ${crmUser.email} already exists in auth, fetching ID...`);

            // Get user by email
            const { data: existingUsers, error: fetchError } = await supabase.auth.admin
              .listUsers();
            ...
            if (!existingUser) {
              throw new Error(`User ${crmUser.email} exists but couldn't be found`);
            }
