...
const ASSET_IMAGE = "https://images.unsplash.com/photo-1586281380349-632531db7ed4?q=80&w=2670&auto=format&fit=crop"; // Placeholder resume/office image
...
export default function Landing() {
  ...
  // ── Panel State: handles BOTH flows ──────────────────────────────────────
  // Flow A (old): ?from=pdf&id=X&mode=chat  (legacy, kept for back-compat)
  // Flow B (new): ?resumeId=X&openChat=true  OR  ?resumeId=X&openVideo=true
  //              Generated by enhanced PDF buttons in FinalResult.tsx
  // ─────────────────────────────────────────────────────────────────────────
  const location = useLocation();

  // --- parse params once for initial state ---
  const _sp = new URLSearchParams(location.search);
  // Flow B params
  const _resumeId = _sp.get('resumeId');
  const _openChat = _sp.get('openChat') === 'true';
  const _openVideo = _sp.get('openVideo') === 'true';
  // Flow A params (legacy)
  const _mode = _sp.get('mode') as 'chat' | 'video' | 'resume' | null;
  const _fromPdf = _sp.get('from') === 'pdf';
  const _id = _sp.get('id');

  // Resolve initial mode
  const _resolvedMode: 'chat' | 'video' | 'resume' =
    _openVideo ? 'video' :
      _openChat ? 'chat' :
        (_mode as any) || 'chat';

  // Resolve initial open state
  const _shouldOpen =
    (!!_resumeId && (_openChat || _openVideo)) ||           // Flow B
    (!!_fromPdf && !!_id && !!_mode);                        // Flow A

  const [isPanelOpen, setIsPanelOpen] = useState(_shouldOpen);
  const [panelMode, setPanelMode] = useState<'chat' | 'video' | 'resume'>(_resolvedMode);
  ...
  const loadResumeData = async (id: string) => {
    ...
    try {
      ...
      const [crmResult, regularResult] = await Promise.all([
        (supabase as any).from('crm_job_requests').select('resume_url, id').eq('id', id).maybeSingle(),
        (supabase as any).from('job_requests').select('resume_path, recordings(storage_path)').eq('id', id).maybeSingle()
      ]);

      if (crmResult.data) {
        setResumeUrl(crmResult.data.resume_url);
        const { data: recordingData } = await (supabase as any)
          .from('crm_recordings')
          .select('video_url')
        ...
        if (recordingData?.video_url) {
          ...
          setVideoUrl(path.startsWith('http') ? path : (supabase as any).storage.from('CRM_users_recordings').getPublicUrl(path).data.publicUrl);
        }
        return;
      }
      ...
      console.error("[DigitalResume] Error loading resume data:", err);
    } finally {
  ...
  useEffect(() => {
    ...
    // ── Flow B: new PDF link params (?resumeId=X&openChat=true) ──
    const resumeId = params.get('resumeId');
    const openChat = params.get('openChat') === 'true';
    const openVideo = params.get('openVideo') === 'true';

    if (resumeId && (openChat || openVideo)) {
      const resolvedMode: 'chat' | 'video' = openVideo ? 'video' : 'chat';
      if (!isPanelOpen || panelMode !== resolvedMode) {
        setPanelMode(resolvedMode);
        setIsPanelOpen(true);
      }
      if (resumeId !== initializedIdRef.current) {
        initializedIdRef.current = resumeId;
        loadResumeData(resumeId);
      }
      return; // don't fall through to Flow A
    }

    // ── Flow A: legacy PDF link params (?from=pdf&id=X&mode=chat) ──
    const mode = params.get('mode') as 'chat' | 'video' | 'resume' | null;
        ...
        <div className="w-full px-8 lg:px-16 py-14">
          <div className="grid md:grid-cols-3 gap-10 mb-10">
            ...
            <div>
              ...
              <div className="space-y-2 text-sm">
                {["Solutions", "Pricing", "Privacy", "Terms", "Careers"].map((link) => (
                  <a key={link} href="#" className="block text-slate-600 hover:text-slate-900 transition-colors">
                    {link}
                  </a>
                ))}
              </div>
              ...
              <div className="flex gap-3">
                {[
                  ...
                  <motion.a
                    key={i}
                    href={social.href}
                    whileHover={{ scale: 1.1, boxShadow: "0 0 18px rgba(148,163,184,0.6)" }}
                    className="w-9 h-9 rounded-full bg-white border border-slate-200 flex items-center justify-center hover:border-cyan-400 transition-all"
                  >
                    <social.icon className="h-4 w-4 text-slate-600" />
                  </motion.a>
