 ...
 * ------------------------------------------------------------------
 * Shared admin console for managing candidate resumes, CRM users, and
 * admins. This is the exact body that previously lived directly in
 * src/pages/DigitalResumeDashboard.tsx — extracted unchanged so it can
 * be composed by BOTH:
 *   - src/pages/DigitalResumeDashboard.tsx        (Tier 1, unchanged route)
 *   - src/features/career-identity/pages/CareerIdentityDashboard.tsx (Tier 2)
 *
 * No internal logic was rewritten during extraction (upload, generate,
 * CRM sync, admin management, analytics all behave exactly as before).
 * The only additions are the `tier` / `title` props below, used purely
 * for branding — this component does not itself branch behavior by
 * tier. Tier-specific behavior (Play Intro → Career Identity profile
 * page) is handled downstream in src/pages/FinalResult.tsx, which reads
 * the tier live from the resume being viewed rather than from whichever
 * dashboard the admin navigated from.
 */
export interface ResumeManagementConsoleProps {
    /** Cosmetic only today. Reserved for future per-tier filtering/behavior. */
    tier: SubscriptionTier;
    /** Heading shown in the top bar, e.g. "Digital Resume CRM" / "Career Identity CRM". */
    title: string;
}
...
export default function ResumeManagementConsole({ tier, title }: ResumeManagementConsoleProps) {
    const { user } = useAuth();
    ...
    return (
        <div
            data-console-tier={tier}
            className="h-screen bg-white flex flex-col overflow-hidden antialiased text-slate-900 font-['-apple-system',_BlinkMacSystemFont,_'Segoe_UI',_Roboto,_Helvetica,_Arial,_sans-serif]"
        >
            {/* Top Admin Nav */}
            <nav className="bg-[#0B4F6C] text-white px-4 md:px-6 py-3 flex justify-between items-center shadow-lg z-20 shrink-0">
                <div className="flex items-center gap-2 md:gap-3">
                    ...
                    <div className="hidden xs:block">
                        <h1 className="text-sm md:text-base font-bold tracking-tight uppercase">{title}</h1>
                    </div>
                </div>
            ...
            <main className="flex-1 p-4 md:p-6 overflow-hidden flex flex-col">
                <div className="max-w-[1600px] mx-auto w-full flex-1 flex flex-col overflow-hidden">
                    ...
                    <div className="flex items-center gap-1 mb-6 bg-slate-100/50 p-1 rounded-xl w-fit shrink-0 max-w-full overflow-x-auto no-scrollbar">
                        ...
                        <button
                            onClick={() => setActiveTab('career-identity')}
                            className={`flex items-center gap-2 px-4 md:px-6 py-2.5 rounded-lg text-sm font-bold transition-all whitespace-nowrap ${activeTab === 'career-identity'
                                ? 'bg-white text-[#0B4F6C] shadow-md border border-slate-200'
                                : 'text-slate-500 hover:text-slate-700 hover:bg-white/50'}`}
                        >
                            <Sparkles className="w-4 h-4" />
